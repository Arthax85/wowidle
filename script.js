...
startBtn.addEventListener("click", () => {
  ...
  if (name.length >= 3 && name.length <= 20) {
    ...
    playerNameDisplay.textContent = `Personaje: ${playerName}`
    saveGameData()
    loadGameData() // Cargar datos después de iniciar el juego
  } else {
...
function renderQuests() {
  ...
  quests
    ...
    .forEach((quest) => {
      ...
      objective.className =
        "quest-objective" + (quest.completed ? " completed" : "")
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Cargar datos solo si el usuario ya ha iniciado el juego antes
  const savedData = localStorage.getItem("gameData")
  if (savedData) {
    const data = JSON.parse(savedData)
    playerName = data.playerName
